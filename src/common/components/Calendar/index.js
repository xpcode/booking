...
export default class Calendar extends Component {
    ...
    renderGridCell(date, num, isFill = false) {
        ...
        const color = (num === 2 ? dataSource[key] : null) || ''

        return (
            <div key={this.state.prefix + num + key}
                className={`calendar-grid-item ${color} ${gray}`}
                onClick={num === 2 ? this.handleClickCell(date) : null}>
                {date.date()}
            </div>
        )
    }
